-- BYLLD X initial schema. Apply only after reviewing this migration in a dedicated Supabase project.
...
-- Safe public-to-members idea summary. Protected detail is kept in a separate table.
...
-- The registered role is immutable; it is not sourced from user-editable JWT metadata.
...
-- Message sending is deliberately not granted directly. A server transaction must verify:
-- active connection, no block, sender Premium, receiver membership and rate limits before insert.
...
('Artificial Intelligence','artificial-intelligence'),('Agriculture','agriculture'),('Climate & Energy','climate-energy'),('Fintech','fintech'),('Healthtech','healthtech'),('Enterprise Software','enterprise-software'),('Consumer','consumer');
